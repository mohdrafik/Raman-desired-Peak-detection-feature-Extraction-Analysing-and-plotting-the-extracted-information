...
%   ydatainv = smooth(ydatainv,navg);
    ydatainv = smooth(ydatainv);
    % use findpeaks to get 
    % change minpeakheight and minpeakwidth to tweak automatic peak
    % detection
   figure
    plot(xdata,ydatainv,'-bo');
     title("normalized and inverted data plot ");

    minpeakheigth = (max(ydatainv)-min(ydatainv))/10; % this is to detect minimum peak approximately (1/20) ~= 0.05 peak
    minpeakwidth = 5;  % earlier it was 3
%     [pks,locs] = findpeaks(ydatainv);  % added thisline Rafik need to see furthder   p= polyfit(x,y,1) 1 is order y_fit = polyval(p,x)
...
%     locs(1)
	fprintf(" value of location jUst to check = %d\n", locs(1));
    finit = xdata((locs(1)));
    ...
    amp = (max(ydata)-min(ydata)); % its ok for FWHM
